...
public class ProfileServiceImpl implements ProfileService {
  ...
  public Profile save(final ProfileDTO profileDTO) {
    val alreadyExist = profileRepository
            .findByEmailAddress(profileDTO.getEmailAddress()).isPresent();
    if (alreadyExist) {
  ...
  public void deleteProfileById(final String profileId) {
    ...
    profileRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("Cannot find profile for specified ID - " + profileId));
